...
| 19 | Receipts: `facility.run.v1` is a compatible superset of the legacy receipt schema | existing repositories can keep emitting their schema for signed ingestion, while control-plane sessions emit the superset. Privacy boundary preserved: metrics only, hashed actors, never prompts/bodies/tool IO. |
