...
export class CommandJournal {
  ...
  push(data: string) {
    ...
    while (newline !== -1) {
      ...
      const frame = JSON.parse(line) as { seq: number; stream: string; data: string };
      if (
...
function write(stream, data) {
  const frame = JSON.stringify({ seq: seq++, stream, data: Buffer.from(data).toString('base64') }) + '\n';
...
child.on('close', code => { fs.closeSync(fd); process.exitCode = code ?? 1; });
`;
