...
> Verification: independent GPT-5.5 xhigh rounds. UX round 1 scored IA 84 / voice 80 / verbs 58 / honesty 72 (verb+honesty findings dominated by the then-unmerged P3 routes). Impl/security round 1 confirmed every P0–P2 claim EXISTS-AND-WIRED (impl 78 / security 62 / honesty 68); its four actionable findings are **closed** (b860133): run keys minted from the agent's clamped permissions, `buildRunBundle` clones the repo named in `run.gh`, `failRun` releases conversation locks, installation discovery refuses project-pinned principals. Round 2 (impl 82 / security 66 / honesty 74) surfaced four more, all **closed** (0e9fecb): the generic proposal route refuses `mcp_tool_call` (that op's per-tool permission is enforced only on the MCP route — closed a hitl:write→privileged-op escalation); conversation turns are pinned to their owning run so a forged run can't release/append to another thread; the harness `TOOLS.md` credential was corrected to `FACILITY_PLATFORM_KEY`; and a stale-turbo-cached lint failure in the markdown renderer was fixed (floor is green under direct biome).
> **Known residual (security, needs a sandbox-architecture change, not a patch):** the runner holds its run-scoped token in its environment, and the engine child runs at the **same UID**, so a compromised agent could read `/proc/<runner-pid>/environ` and mint a `contents:write` push token for the run's own repo. The push token is already single-repo + contents-only and merges stay human-gated, but full closure needs a distinct engine UID (or `hidepid`/gVisor) in the sandbox image — an e2e-gated change tracked with the owner-gated GitHub App production install.
> **Open product follow-ups:** harness item creation + test-run + where-used/diffs, KB in-place editing + validation UI, issue stage filters + PR mirror, CLI/MCP parity for the new verbs, roles editor + sandbox-profile UI, markdown rendering for KB/harness content, per-project SSE channel (LiveRefresh polls today). **Owner-gated:** GitHub App install, tam-os cutover + the §0 week-long dogfood, branch push/PR.
>
> **Strategy layer (2026-07-07).** Adrián's second product review (Vercel comparison) ratified a design method and laws on top of this spec — questions-first hierarchy, workable lists, semantic controls, type-specific editors, the pipeline view, measurement in product. They live in [.claude/skills/control-plane-strategy/SKILL.md](../../.claude/skills/control-plane-strategy/SKILL.md); read that skill before building any surface in this document. Its §"Ratified deltas" extends the targets below.
>
> **Strategy-layer implementation (2026-07-07, same branch — all seven deltas landed, verified live on the tam-os mirror).** W1 semantic backbone: `GET /v1/catalog` (engines, models from pricing, permissions, wired trigger kinds), `GET /v1/projects/:id/agents/status` (server-computed nextRunAt via the scheduler's own parser, last/live run, 14d counts + PRs, contract description, integration event bindings), integrations CRUD + deliveries endpoint, runs `agentDefId` filter; SDK regen + typed contracts (e23aa41). W2 agents flagship: engine view (health strip, search/facets, schedules in words, next fire, 14d bars) + agent detail (run now, inline contract draft→publish, schedule builder, permission matrix, engine/model/sandbox pickers with price hints, session history, arm-confirm delete) + one-screen creation; free-text model/cron/permission inputs are gone (54c1f46). W3 the loop: trigger→agent→output rendered from live state, real edges only, compact variant on the overview (ef6eb55). W4 lists to L3: sessions/fleet share a workspace table (24h decision strip, search, status/agent facets, PR column), harness gains search/scope facets + in-product item creation, issues search + label facets, audit summary strip over the loaded window (b9bbbcc). W5 measurement in product: overview engine module, spend-by-agent names, shipped panel with one-shot/fixup annotations (a58944e). W6 integrations UI: event sources with sealed secrets, project + agent routing pickers, webhook endpoint display, recent deliveries (93615f5). Full floor green each wave (142 api tests incl. 4 new, direct biome repo-wide, web tsc); browser-verified per wave including a full create→schedule→delete agent loop and a settings→integration→agent-binding round trip.
