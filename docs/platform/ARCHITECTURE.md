...
- **HITL (AUTO-202)**: `action_types` (name, payload JSON schema, resolver_type emails|permission|team|dynamic, executor internal|webhook|none), `proposals` (action_type, payload, context_md, project, run, current_state), `proposal_events` (append-only: draft→open→approved|rejected|cancelled|expired, executed|execution_failed).
...
- **Observation**: `audit_events` (append-only, hash-chained, actor human|agent|system, action, target, payload), `platform_issues` (kind drift|budget_breach|run_failure|stuck_session|guard_failure|canary_failure|integration_error, fingerprint dedupe, state open|acked|resolved), `outcomes` (agent PR fate: merged|rejected, review rounds, fixup commits, hours_to_terminal), `analytics_daily` (per project × agent × model rollups).
